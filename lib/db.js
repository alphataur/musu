...
class fastStore{
  constructor(){
    ...
    this.analytics = redis.createClient({ database: 4 })
    this.createdAt = new Date()
  }
  async addTickSong(id){
    if(await this.analytics.exists(id))
      let seconds = await this.analytics.HGETALL(id, "seconds")
    else
      let seconds = 0
    await this.analytics.HSET(id, "seconds", seconds+1)
  }
  async addPlaySong(id){
    if(await this.analytics.exists(id))
      let iters = await this.analytics.HGET(id, "iters")
    else
      let iters = 0
    await this.analytics.HSET(id, "iters", iters)
  }
  async _getAnalytics(){
